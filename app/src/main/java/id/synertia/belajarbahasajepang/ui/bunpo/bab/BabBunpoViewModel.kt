...
@HiltViewModel
class BabBunpoViewModel @Inject constructor(
    private val repository: APIRepository,
    private val networkHelper: NetworkHelper
) : ViewModel() {

    private val _response = MutableLiveData<Resource<List<BabBunpo>>>()
    val response: LiveData<Resource<List<BabBunpo>>>
        get() = _response
    ...
    fun get(id:String) {
        viewModelScope.launch {
            ...
            if (networkHelper.isNetworkConnected()) {
                repository.getBabBunpo(id).let { response ->
                    if (response.isSuccessful) {
                        val body = response.body()
                        if (body != null && !body.error) {
                            val data = body.subbunpo
