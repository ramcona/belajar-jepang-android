...
@HiltViewModel
class PenjelasanViewModel @Inject constructor(
    private val repository: APIRepository,
    private val networkHelper: NetworkHelper
) : ViewModel() {

    private val _response = MutableLiveData<Resource<List<Penjelasan>>>()
    val response: LiveData<Resource<List<Penjelasan>>>
        get() = _response
    ...
    fun get(id:String) {
        viewModelScope.launch {
            ...
            if (networkHelper.isNetworkConnected()) {
                repository.getPenjelasan(id).let { response ->
                    if (response.isSuccessful) {
                        val body = response.body()
                        if (body != null && !body.error) {
                            val data = body.penjelasan
