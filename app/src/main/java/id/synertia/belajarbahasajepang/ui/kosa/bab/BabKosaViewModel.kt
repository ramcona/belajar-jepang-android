...
@HiltViewModel
class BabKosaViewModel @Inject constructor(
    private val repository: APIRepository,
    private val networkHelper: NetworkHelper
) : ViewModel() {

    private val _response = MutableLiveData<Resource<List<BabKosa>>>()
    val response: LiveData<Resource<List<BabKosa>>>
        get() = _response
    ...
    fun get(id:String) {
        viewModelScope.launch {
            ...
            if (networkHelper.isNetworkConnected()) {
                repository.getBabKosa(id).let { response ->
                    if (response.isSuccessful) {
                        val body = response.body()
                        if (body != null && !body.error) {
                            val data = body.subkosa
